...
function RegisterEvent() {
  ...
  const [formData, setFormData] = useState({
    ...
    dataConsent: false,
    pending: 'pending',
    registeredDate: new Date().toLocaleString("en-US", {timeZone: "Asia/Singapore"})
  })
  ...
  const checkRegistered = (event) => {
    let eventIdArr = []
    eventIdArr = user.data.registeredEvents.flatMap((obj) => eventIdArr.concat(Object.keys(obj)))
  ...
  const registrationDetails = (eventId, param) => {
    for (let i = 0; i < user.data.registeredEvents.length; i++) {
      ...
      if (Object.keys(eventObj)[0] === eventId) {
        return eventObj[eventId][param] === '' ? 'N/A' : eventObj[eventId][param]
      }
    }
  }
        ...
        <form onSubmit={onSubmit} key={event} >
          <VStack spacing={8} align={'flex-start'} >
            <Grid w={'100%'} templateColumns='repeat(5, 1fr)' gap={4}>
              <GridItem colSpan={4}>
                <Text fontSize={'lg'} fontWeight={700}>{'Register: ' + event.name}</Text>
              </GridItem>
              <GridItem colSpan={1}>
                {user ? (
                  <Tag size={'sm'} variant='subtle' borderRadius={'full'}>
                    ...
                    <TagLabel>{checkRegistered(event) ? 'Registered' : 'Not Registered'}</TagLabel>
                  </Tag>
                ) : (<></>)}
              </GridItem>
            </Grid>
            ...
            {user ? (
              <>
                <VStack spacing={4} align={'flex-start'} >
                  <Alert status='info' borderRadius={'lg'} borderColor={'accents.blue'} borderWidth={'thin'} >
                    <AlertIcon color={'accents.blue'} />
                    Upon registration and receipt of the registration fee, you will receive a confirmation email with the race instructions by the following day. 
                    You may then proceed to time your 5km run using the Strava app and upload it to result upload page.
                  </Alert>
                </VStack>
                
                {checkRegistered(event) ? (
                  <VStack spacing={4} align={'flex-start'} >
                    <Text fontWeight={700} fontSize={'md'} >Registration details</Text>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >Status:</Text>
                      <Text fontWeight={400} >{'Registration ' + registrationDetails(event._id, 'pending')}</Text>
                    </HStack>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >Registered on:</Text>
                      <Text fontWeight={400} >{registrationDetails(event._id, 'registeredDate')}</Text>
                    </HStack>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >Category:</Text>
                      <Text fontWeight={400} >{registrationDetails(event._id, 'category')}</Text>
                    </HStack>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >Institution:</Text>
                      <Text fontWeight={400} >{registrationDetails(event._id, 'institution')}</Text>
                    </HStack>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >Name of parent / guardian:</Text>
                      <Text fontWeight={400} >{registrationDetails(event._id, 'parentName')}</Text>
                    </HStack>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >NRIC of parent / guardian:</Text>
                      <Text fontWeight={400} >{registrationDetails(event._id, 'parentNRIC')}</Text>
                    </HStack>

                    <HStack spacing={4} fontSize={'sm'} >
                      <Text fontWeight={700} >Mobile no. of parent / guardian:</Text>
                      <Text fontWeight={400} >{registrationDetails(event._id, 'parentMobile')}</Text>
                    </HStack>
                  </VStack>
                ) : (
                  <>
                    <VStack spacing={4} align={'flex-start'} >
                      <Text fontSize={'lg'} fontWeight={700}>Personal Particulars</Text>
                      <Text fontSize={'sm'} fontWeight={400}>You can edit your personal particulars in the My Account page</Text>
                      <HStack spacing={4} fontSize={'sm'} >
                        <Text fontWeight={700} >Name:</Text>
                        <Text fontWeight={400} >{user.data.firstName + ' ' + user.data.lastName}</Text>
                      </HStack>
                      <HStack spacing={4} fontSize={'sm'} >
                        <Text fontWeight={700} >DOB:</Text>
                        <Text fontWeight={400} >{formatDate(user.data.birthDate)}</Text>
                      </HStack>
                      <HStack spacing={4} fontSize={'sm'} >
                    ...
                    <FormControl isRequired>
                      ...
                      <Select name='category' onChange={onChange} placeholder='Select category'>
                    ...
                    <FormControl isRequired>
                      ...
                      <Input name='institution' value={institution} placeholder='Institution' onChange={onChange} />
                    </FormControl>

                    <Divider borderColor={"primary.800"} opacity={1} borderBottomWidth={1.5} />

                    <VStack spacing={4} align={'flex-start'}>
                      <Text fontSize={'lg'} fontWeight={700}>PAR-Q Assessment</Text>
                    ...
                    <VStack spacing={4} align={'flex-start'}>
                      <Text fontSize={'lg'} fontWeight={700}>COVID-19 Safety Acknowledgement</Text>
                      <Text fontSize={'sm'} fontWeight={400}>
                        I agree to self-monitor for signs and symptoms of COVID-19 (symptoms typically include fever, cough, and shortness of breath). 
                        I will not run in MacRitchie Reservoir Park if I or my close contacts have quarantine order, stay-home notice or discover the mentioned symptoms of COVID-19 in the past 14 days. 
                        I have read and accept to{' '}
                        <Link color='teal.500' href='https://www.moh.gov.sg/covid-19/resources'>
                    ...
                    <VStack spacing={4} align={'flex-start'}>
                      <Text fontSize={'lg'} fontWeight={700}>Indemnity Agreement</Text>
                      <Text fontSize={'md'} fontWeight={700} >Conditions of entry</Text>
                      <Text fontSize={'sm'} fontWeight={400} >
                        I assume and accept full responsibility for the risks of my participation in the event, 
                        and for any injury, damage, death, or other loss that I may suffer, resulting from those risks, 
                        include but not limited to the risk of a third party's passive or active negligence or other misconduct.
                      </Text>
                      <FormCheckbox 
                        name='termsAndCond'
                        data={termsAndCond}
                      ...
                      <Text fontSize={'md'} fontWeight={700} >Parent's or guardian's consent</Text>
                      <FormRadio 
                        question='' 
                        name='above18' 
                        data={above18} 
                      ...
                      <FormControl>
                        ...
                        <Input name='parentName' value={parentName} onChange={onChange} />
                      </FormControl>
                      <FormControl>
                        <FormLabel>NRIC / FIN of parent or guardian (last 3 digits and ending alphabet)</FormLabel>
                        <Input name='parentNRIC' value={parentNRIC} onChange={onChange} />
                      </FormControl>
                      <FormControl>
                        <FormLabel>Mobile number of parent or guardian</FormLabel>
                        <Input name='parentMobile' value={parentMobile} onChange={onChange} />
                      </FormControl>
                    </VStack>
                    ...
                    <Button type='submit' size={'lg'} w='100%' bg={'primary.800'} color={'primary.white'} >Submit Registration</Button>
