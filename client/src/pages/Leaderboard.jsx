...
function Leaderboard() {
  ...
  function search(results) {
    ...
    if (filterParam.eventType === 'Individual') {
      ...
      for (let [inst, resultsArr] of Object.entries(groupedResults)) {
        ...
        let duplicatesRemoved = sortedResArr.filter((result) => {
          if (!uniqueIds.includes(result.userId)) {
            uniqueIds.push(result.userId)
            return true
          }
  ...
  if (isLoading || !(events.length > 0)) {
    ...
    return (
      <Stack spacing={8}>
        <Text fontWeight={700} fontSize={'2xl'} >Leaderboard</Text>
  
        <Grid w={'100%'} templateColumns='repeat(2, 1fr)' gap={4}>
          <GridItem colSpan={1}>
            <Text fontWeight={700} fontSize={'sm'}>Category</Text>
          ...
          <GridItem colSpan={1}>
            <Text fontWeight={700} fontSize={'sm'}>Individual or Team</Text>
